...
public class VoterLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(rs1.next()) {
			...
			PreparedStatement st=con.prepareStatement(" select * from register where vNumber='"+vNumber+"'  ");
			ResultSet rs=((java.sql.Statement) st).executeQuery("select * from register where vNumber='"+vNumber+"'  ");		
		if(rs.next()) {
